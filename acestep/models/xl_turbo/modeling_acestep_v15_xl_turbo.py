    ...
    def generate_audio(
        ...
        **kwargs,
        ...
        for step_idx in range(num_steps):
            ...
            if step_idx in (0, num_steps // 2, num_steps - 1):
                _xt_min = xt.min().item()
                _xt_max = xt.max().item()
                ...
                logger.info(
                    f"[extend-trace] xt stats step {step_idx}/{num_steps} "
                    f"t_after={t_after_step:.4f} "
            ...
            if repaint_mask is not None and clean_src_latents is not None and step_idx < injection_cutoff:
                if step_idx in (0, injection_cutoff - 1) or (step_idx == 1 and injection_cutoff > 2):
                    ...
                    logger.info(
                        f"[extend-trace] step-inject step {step_idx}/{num_steps} "
